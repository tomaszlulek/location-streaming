...
object KinesisConsumer {
  ...
  def make[F[_] : Async](): Resource[F, Kinesis[F]] =
    for {
      k <- KinesisInterpreter[F].KinesisAsyncClientResource(KinesisAsyncClient.builder())
      d <- DynamoDbInterpreter[F].DynamoDbAsyncClientResource(DynamoDbAsyncClient.builder())
      c <- CloudwatchInterpreter[F].CloudWatchAsyncClientResource(CloudWatchAsyncClient.builder())
